...
<%
	...
	dto = (GoodsDTO)request.getAttribute("goodsRetrieve");
...
<script type="text/javascript">
	$(document).ready(function(){
		 $("#up").on("click",function(){
			var up = $("#GOODS_AMOUNT").val();
			$("#GOODS_AMOUNT").val(Number(up)+1);
			
		});
		$("#down").on("click",function(){
			if($("#GOODS_AMOUNT").val()>1){
				var down = $("#GOODS_AMOUNT").val();
				$("#GOODS_AMOUNT").val(Number(down)-1);
			}
		}); 
		$("#cart").on("click",function(){
			if($("#GOODS_SIZE").val()=="사이즈선택"){
...
<FORM name="goodRetrieveForm" method="GET" action="#">
	    <input type="hidden" name="gImage" value="<%=dto.getGimage() %>"> <input
		type="hidden" name="gCode" value="<%=dto.getGcode()%>"> <input
		type="hidden" name="gName" value="<%=dto.getGname()%>"> <input
		type="hidden" name="gPrice" value="<%=dto.getGprice()%>">
		...
		<tr>
			<td>
				<table align="center" width="710" cellspacing="0" cellpadding="0"
					...
					<tr>
						<td class="td_default"><font size="5"><b><%=dto.getGcontent() %></b></font>
							&nbsp;</td>
					</tr>
					<tr>
						<td height="5"></td>
					...
					<tr>
						...
						<td class="td_default" colspan="2" style='padding-left: 30px'><%=dto.getGcode() %>
						</td>
					</tr>
					<tr>
