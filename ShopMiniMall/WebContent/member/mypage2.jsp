...
<body>
<%
	MemberDTO dto = (MemberDTO)session.getAttribute("mypage");
...
<form action="MemberUpdateServlet" method="post">
	아이디 : ${login.userid}<br>
	<input type="hidden" value=<%=userid %> name="${login.userid}">
	이름 : ${login.username}<br>
	우편번호 : <input type="text" id="sample4_postcode" placeholder="우편번호" name="post" value=<%=post %>><br>
	도로명주소 : <input type="text" value="${login.addr1}" id="sample4_roadAddress" placeholder="도로명주소" name="addr1"><br>
	지번주소 : <input type="text" value="${login.addr2}" id="sample4_jibunAddress" placeholder="지번주소" name="addr2"><br>
	<span id="guide" style="color:#999"></span><br>
	전화번호 : <select name="${login.phone1}">
		<option value="011" <%if("011".equals(phone1)){ %>selected<%} %>>011</option>
		<option value="010" <%if("010".equals(phone1)){ %>selected<%} %>>010</option>
		<option value="017" <%if("017".equals(phone1)){ %>selected<%} %>>010</option>
	</select>-
	<input type="text" name="phone2" value=<%=phone2 %>>-<input type="text" name="phone3" value=<%=phone3 %>><br>
	이메일 : <input type="text" name="email1" value=<%=email1 %>>@<input type="text" name="email2" placeholder="직접입력" value=<%=email2 %>>
	<select name="email">
